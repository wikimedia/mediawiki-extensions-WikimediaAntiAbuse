...
const APP_SELECTOR = '.mw-wikimediaantiabuse-abuse-review-row-actions-app';
const ROW_SELECTOR = '.mw-wikimediaantiabuse-abuse-review-row';
const ID_PREFIX = 'mw-wikimediaantiabuse-abuse-review-row-';

/**
 * Mount one app per row. Only a row's actions are Vue; the rest of the queue arrives as
...
function mountRowActions() {
	...
	Array.prototype.forEach.call( document.querySelectorAll( APP_SELECTOR ), ( mountPoint ) => {
		let props;
		try {
			props = JSON.parse( mountPoint.getAttribute( 'data-actions' ) );
		} catch ( error ) {
			// One unreadable row must not cost the rest of the queue its actions.
			mw.log.warn( 'Skipping a review row with unreadable actions: ' + error );
			return;
		}
		// JSON.parse( null ) is null rather than a throw, so a missing attribute lands here.
		if ( !props ) {
			mw.log.warn( 'Skipping a review row with no actions payload' );
			return;
		}
